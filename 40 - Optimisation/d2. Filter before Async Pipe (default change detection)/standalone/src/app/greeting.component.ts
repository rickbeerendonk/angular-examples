...
@Component({
  selector: 'greeting',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.Default, // Default change detection
  imports: [AsyncPipe],
  template: `<h1>Hello {{ (value$ | async) ?? 0 }}</h1>`
})
  ...
  implements AfterContentChecked, AfterViewChecked
{
  value$ = getData().pipe(
    ...
    /* DistinctUntilChanged is needed */ distinctUntilChanged()
  );
